...
<form action="/teacher/lectures/${lecture.id}/delete" method="post" onsubmit="return confirm('Delete?')"><input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/><button class="btn btn-outline-danger btn-sm">&#128465;</button></form></div></c:forEach></c:otherwise></c:choose>
</div></div></div>
...
<form action="/teacher/polls/${poll.id}/delete" method="post" onsubmit="return confirm('Delete?')"><input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/><button class="btn btn-outline-danger btn-sm">&#128465;</button></form></div></c:forEach></c:otherwise></c:choose>
</div></div></div>
</div>
